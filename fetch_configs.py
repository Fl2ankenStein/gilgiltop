name: جمع‌آوری و تست کانفیگ VLESS
...
jobs:
  fetch-configs:
    ...
    steps:
      ...
      - name: رمزگشایی session
        run: |
          if [ -f session.session.enc ]; then
            python -c "
import os
from cryptography.fernet import Fernet
key = os.getenv('SESSION_DECRYPT_KEY').encode()
with open('session.session.enc', 'rb') as f:
    data = f.read()
cipher = Fernet(key)
with open('session.session', 'wb') as f:
    f.write(cipher.decrypt(data))
print('✅ session.session با موفقیت ایجاد شد.')
"
          else
            echo '⚠️ فایل session.session.enc یافت نشد.'
          fi
      ...
      - name: اجرای اسکریپت
        env:
          ...
          GH_FILE_PATH: ${{ secrets.GH_FILE_PATH }}
